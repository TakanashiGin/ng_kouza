...
[iscript]
/* 初期化 */
f.font_color    = mp.font_color    || "0x383C5F"; // テキストのフォントカラー
f.name_color    = mp.name_color    || "0xF5F5F5"; // 名前欄のフォントカラー
f.frame_opacity = mp.frame_opacity || "255"; // メッセージフレームの不透明度（0～255）
[endscript]

;-----------------------------------
;▼メッセージウィンドウを表示するマクロ（faceなし）
;-----------------------------------
[macro name="message_window_adventure"]
...
[if exp="mp.face == 'true'"]
  [ptext name="chara_name_area" layer="message0" color="&f.name_color" size=32 x=0 y=410 width="285" align="right"]
  [chara_config ptext="chara_name_area"]
  [position layer="message0" page="fore" frame="../others/plugin/theme_kopanda_03b/image/frame_message.png" margint="70" marginl="180" marginr="50" marginb="30" opacity="&f.frame_opacity"]
[else]
  [ptext name="chara_name_area" layer="message0" color="&f.name_color" size=28 x=10 y=410 width="300" align="center"]
  [chara_config ptext="chara_name_area"]
  [position layer="message0" page="fore" frame="../others/plugin/theme_kopanda_03b/image/frame_message.png" margint="70" marginl="40" marginr="50" marginb="30" opacity="&f.frame_opacity"]
[endif]

[font color="&f.font_color"]
[deffont color="&f.font_color"]
